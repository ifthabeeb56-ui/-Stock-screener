...
def analyze_stock(ticker, f_ema, s_ema, rsi_min):
    try:
        ...
        return {
            ...
            "RSI": round(c_rsi, 1), "Vol Ratio": round(curr_vol/avg_vol, 2) if avg_vol > 0 else 0,
...
def main():
    ...
    # Default ആയി 100 എണ്ണം സെലക്ട് ചെയ്യുന്നു, നിങ്ങൾക്ക് 500 വരെ നീക്കാം
    scan_limit = st.sidebar.slider("എത്ര സ്റ്റോക്കുകൾ സ്കാൻ ചെയ്യണം?", 1, max_limit, min(100, max_limit))
    ...
    if st.button("🚀 Start Scanning", use_container_width=True):
        ...
        if results:
            ...
            st.download_button(label="📥 റിപ്പോർട്ട് ഡൗൺലോഡ് ചെയ്യുക (CSV)", data=csv,
                               file_name=f'habeeb_signals_{index_name.replace(" ", "_")}.csv', mime='text/csv')
        else:
